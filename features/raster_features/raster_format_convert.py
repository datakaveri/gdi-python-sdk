...
RASTER_DRIVER_MAPPING = {".tif": "GTiff", ".tiff": "GTiff", ".img": "HFA"}
...
def convert_raster_format(
    ...
    if input_store not in ("local", "minio") or output_store not in ("local", "minio"):
        raise ValueError(
            "input_store and output_store must be either 'local' or 'minio'."
        )
    ...
    if input_store == "local":
        ...
        resp = client.get_object(bucket_name, input_path)
    ...
    try:
        ...
        result = gdal.Translate(
            local_output_path, local_input_path, options=translate_options
        )
        ...
        if output_store == "local":
            ...
            stream_to_minio(client, bucket_name, output_path, local_output_path)
