...
# def _fix_nodata_and_reproject(raster_path, tmp_dir, python_exec, scripts_dir, target_epsg=7755, nodata=-9999):
#     ds = gdal.Open(raster_path)
#     if ds is None:
#         raise FileNotFoundError(f"Cannot open raster: {raster_path}")
...
#         nd = os.path.join(tmp_dir, f"nodata_{base}")
#         subprocess.run(["gdal_translate", "-a_nodata", str(nodata), work_file, nd], check=True)
#         work_file = nd

#     # Reproject
#     if epsg != str(target_epsg):
#         rp = os.path.join(tmp_dir, f"reproj_{base}")
#         subprocess.run([
#             "gdalwarp", "-t_srs", f"EPSG:{target_epsg}", "-r", "near",
#             "-dstnodata", str(nodata), work_file, rp
#         ], check=True)
#         work_file = rp
...
#     fill_script = os.path.join(scripts_dir, "gdal_fillnodata.py")
#     filled = os.path.join(tmp_dir, f"filled_{base}")
#     subprocess.run([python_exec, fill_script, "-md", "5", work_file, filled], check=True)

#     return filled


# def _compute_pairwise_slope(r1, r2, out_file, delta_t_days, python_exec, scripts_dir, nodata=-9999):
#     """Compute slope = (r2 - r1) / Δt."""
#     calc_script = os.path.join(scripts_dir, "gdal_calc.py")
...
#         "-A", r2, "-B", r1,
#         "--outfile=" + out_file,
#         f"--calc=where((A!={nodata}) & (B!={nodata}), (A-B)/{delta_t_days}, {nodata})",
#         "--type=Float32",
...
# def _median_stack(rasters, output):
#     ref = gdal.Open(rasters[0])
#     geotrans = ref.GetGeoTransform()
#     proj = ref.GetProjection()
#     arrs = []
...
#     arr_stack = np.stack(arrs, axis=0)
#     med = np.nanmedian(arr_stack, axis=0)

#     driver = gdal.GetDriverByName("GTiff")
#     out = driver.Create(output, ref.RasterXSize, ref.RasterYSize, 1, gdal.GDT_Float32)
#     out.SetGeoTransform(geotrans)
#     out.SetProjection(proj)
#     b = out.GetRasterBand(1)
#     b.WriteArray(np.where(np.isnan(med), -9999, med))
#     b.SetNoDataValue(-9999)
#     out = None


# # -------------------------------------------------
# # Main Processing (REWRITTEN in correlation format)
# # -------------------------------------------------
...
# ):
#     """
#     Compute Sen’s slope from a CSV containing:
#     filepath, datetime

#     Rewritten to follow EXACT structure:
#     - Step 4: COG conversion
#     - Step 5: Save to local/minio
#     - Step 6: Cleanup
#     """

#     # -----------------------------
#     # Step 1: Connect + Read CSV
#     # -----------------------------
#     client = connect_minio(config, client_id)
...
#         raise ValueError("CSV must have 'filepath' column")
...
#     tmp_dir = tempfile.mkdtemp(prefix="senslope_")
#     python_exec = sys.executable
#     scripts_dir = os.path.join(os.path.dirname(python_exec), "Scripts")
...
#         for fp in df["filepath"]:
#             obj = client.get_object(client_id, fp)
#             local_fp = os.path.join(tmp_dir, os.path.basename(fp))
#             with open(local_fp, "wb") as f:
#                 shutil.copyfileobj(obj, f)
#             local_rasters.append(local_fp)

#         # -----------------------------
#         # Step 3: Fix NoData + Reproject
#         # -----------------------------
#         cleaned = [
#             _fix_nodata_and_reproject(r, tmp_dir, python_exec, scripts_dir)
#             for r in local_rasters
#         ]
...
#         # -----------------------------
#         delta_t = 1  # days difference
#         for i in range(len(cleaned) - 1):
#             out_slope = os.path.join(tmp_dir, f"slope_{i}.tif")
#             _compute_pairwise_slope(cleaned[i], cleaned[i + 1], out_slope, delta_t, python_exec, scripts_dir)
#             slope_files.append(out_slope)

#         # -----------------------------
#         # Step 5: Median stack (Sen's slope result)
#         # -----------------------------
#         final_raw = os.path.join(tmp_dir, "sens_slope.tif")
#         _median_stack(slope_files, final_raw)
